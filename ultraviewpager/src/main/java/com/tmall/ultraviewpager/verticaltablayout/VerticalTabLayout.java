package com.tmall.ultraviewpager.verticaltablayout;
...
import com.tmall.ultraviewpager.verticaltablayout.util.DisplayUtil;
import com.tmall.ultraviewpager.verticaltablayout.util.TabFragmentManager;
import com.tmall.ultraviewpager.verticaltablayout.widget.QTabView;
import com.tmall.ultraviewpager.verticaltablayout.widget.TabView;
...
public class VerticalTabLayout extends ScrollView {
    ...
    /**
     * 创建Tab的监听器，返回自定义Tab的View
     */
    private OnNewTabListener onNewTabListener = new OnNewTabListener() {
        @Override
        public TabView onCreate(TabAdapter adapter, int position) {
            return new QTabView(mContext).setIcon(adapter.getIcon(position))
                    .setTitle(adapter.getTitle(position)).setBadge(adapter.getBadge(position))
                    .setBackground(adapter.getBackground(position));
        }
    };
...
//    }
    ...
    public void setTabAdapter(TabAdapter adapter) {
        ...
        if (adapter != null) {
            ...
            for (int i = 0; i < adapter.getCount(); i++) {
                addTab(onNewTabListener.onCreate(mTabAdapter, i));
            }
        }
    }

    /**
     * 设置创建Tab的监听器，返回自定义Tab的View
     * @param listener
     */
    public void setOnNewTabListener(OnNewTabListener listener) {
        if (listener == null)
            throw new NullPointerException("OnNewTabListener cannot be empty");
    ...
    public interface OnNewTabListener {
        TabView onCreate(TabAdapter adapter, int position);
    }
}
